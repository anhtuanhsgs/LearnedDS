...
def init_linear_lstm (hidden_feats, gpu_id):
    if gpu_id >= 0:
        with torch.cuda.device (gpu_id):
            cx = Variable (torch.zeros (1, hidden_feats).cuda ())
            hx = Variable (torch.zeros (1, hidden_feats).cuda ())
    else:
        cx = Variable (torch.zeros (1, hidden_feats))
        hx = Variable (torch.zeros (1, hidden_feats))
    return cx, hx    
